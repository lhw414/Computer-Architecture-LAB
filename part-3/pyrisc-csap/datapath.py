...
class EX(Pipe):
    ...
    def log(self):
        ...
        ALU_OPS = {
            ...
            # add custom instruction's log
            ALU_SVUNPACK    : f'# {self.alu_out:#010x} <- vectorize {self.op1_data:#010x}',
            ALU_SVPACK      : f'# {self.alu_out:#010x} <- unpack {self.op1_data:#010x}',
            ALU_SVBRDCST    : f'# {self.alu_out:#010x} <- broadcast {self.op1_data:#010x}',
            ALU_SVADDI      : f'# {self.alu_out:#010x} <- V.{self.op1_data:#010x} + IMM.{self.alu2_data:#010x}',
            ALU_SVADD       : f'# {self.alu_out:#010x} <- V.{self.op1_data:#010x} + V.{self.alu2_data:#010x}',
            ALU_SVSUB       : f'# {self.alu_out:#010x} <- V.{self.op1_data:#010x} - V.{self.alu2_data:#010x}',
            ALU_SVMUL       : f'# {self.alu_out:#010x} <- V.{self.op1_data:#010x} * V.{self.alu2_data:#010x}',
        }
